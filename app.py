...
st.markdown("""
    ...
    .value-text { color: #1e293b; font-size: 0.85rem; font-weight: 600; }
    </style>
...
def load_data():
    ...
    try:
        ...
        # Filtro de Responsáveis Únicos (Famílias)
...
if df_geral is not None:
    ...
    # Aplica Filtros
    df_filtrado_fam = df_familias[(df_familias[col_trab].isin(f_trab)) & (df_familias[col_benef].isin(f_benef))]
    # Pega todos os participantes dessas famílias
    df_participantes = df_geral[df_geral["NOME DO RESPONSÁVEL"].isin(df_filtrado_fam["NOME DO RESPONSÁVEL"])]
    ...
    with col_m1:
        st.markdown(f"""<div class="metric-container">
            <p style="margin:0; color:#64748b; font-weight:bold;">UNIDADES FAMILIARES</p>
            <h2 style="margin:0; color:#1e293b;">{len(df_filtrado_fam)}</h2>
        </div>""", unsafe_allow_html=True)
    with col_m2:
        st.markdown(f"""<div class="metric-container">
            <p style="margin:0; color:#64748b; font-weight:bold;">PESSOAS EM ATIVIDADE</p>
            <h2 style="margin:0; color:#1e293b;">{len(df_participantes)}</h2>
        </div>""", unsafe_allow_html=True)

    # --- 6. SELEÇÃO E PRONTUÁRIO ---
    st.write("###")
    lista_nomes = sorted(df_filtrado_fam["NOME DO RESPONSÁVEL"].tolist())
    selecionado = st.selectbox("🎯 Localizar Responsável:", ["SELECIONE UM NOME..."] + lista_nomes)
        ...
        for i, nome_col in enumerate(df_geral.columns):
            with cols[i % 4]:
                st.markdown(f'''<div class="info-card">
                    ...
                    <div class="value-text">{principal[nome_col]}</div>
                </div>''', unsafe_allow_html=True)

        # Tabela de Pessoas da Família em Atividade
    ...
    # --- 7. EXPORTAÇÃO ---
    if st.sidebar.button("➕ Adicionar Selecionado à Lista"):
        if selecionado != "SELECIONE UM NOME...":
    ...
    if st.session_state.lista_exportacao:
        ...
        if st.sidebar.button("📥 Gerar Planilha de Saída"):
            df_exp = df_geral[df_geral["NOME DO RESPONSÁVEL"].isin(st.session_state.lista_exportacao)]
            buf = BytesIO()
            with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
                df_exp.to_excel(writer, index=False)
            st.sidebar.download_button("Clique para Baixar", buf.getvalue(), "Relatorio_Focalizado.xlsx")
else:
    st.info("Aguardando detecção da planilha na pasta...")
